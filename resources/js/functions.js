...
var e = {
    ...
    trafficsourcesChart: function () {
        ...
        //const labels = referrals.map(item => item.referrer);

        if (typeof referrals === 'undefined') {
            return;
        }

        const labels = referrals.map(item => {
            return translatedReferrer[item.referrer] || item.referrer;
        });
        const count = referrals.map(item => item.count);

        var ac = e.select('#apexChartTrafficSources');
        if (e.isVariableDefined(ac)) {
            var options = {
                colors: [
                    '#2163e8', '#0cbc87', '#d6293e', '#f7c32e', '#8925da', "#2ef7d9",
                ],
                labels: labels,
                series: count,
                legend: {
    ...
    trafficstatsChart: function () {
        ...
        const labels = trafficStats.map(item => item.yearMonth);
        const count = trafficStats.map(item => item.count);
